...
Привет. В данном репозитории представлено решение задачи на тему "TF-IDF". Надеюсь, будет интересно! 😉 Мои контакты можно найти в самом низу.
...
```
Касаемо терминов:
- `tf` — это частота слова к количеству слов.
- `idf` — это логарифм от частотного деления количества документов на количество документов с определенным словом.
...
Ну теперь давай посмотрим на код. И начнем мы с frontend. Но важно, посмотри в конце инструкцию по запуску из `VS Code` на `Windows`.
...
Это основной код для отображения всех необходимых кнопок и приема файла. Затем к типу `submit` будет привязана функция JS для отправки все на серверную часть и приема результата обратно. Также в этом участке содержится оформление таблицы. Теперь перейдем к скрипту, отвечающему за обработку и отправку запроса.
```
...
Выше я проверяю, если нет слова как ключа в `hash_count_word`, то добавляем со значением 1, в противном случае увеличиваем значение на 1, то есть это счетчик слов.
...
Закончив цикл `for word...`, но не закончив `for text...`, мы обрабатываем данные по слову и записываем как значение частное от деления количества данного слова в абзаце на количество слов. Это и есть `tf`. Но общий параметр `tf` я сделал как сумму `tf` из каждого абзаца.
...
Файл для примера лежит в папке `Examples` с именем `Example.txt`. Текст в нем такой:
```
Весенний день наполнил город яркими красками. Солнечные лучи играли в каплях дождя, создавая радугу на мокром асфальте. Прохожие улыбались, наслаждаясь теплом после долгой зимы. В парках распускались первые цветы, наполняя воздух нежным ароматом. Дети запускали воздушных змеев, смеясь и крича от восторга. Жизнь била ключом, даря надежду и радость каждому.
Весна пришла в город. Птицы поют в парке. Дети играют на улице. Солнце светит ярко. Люди гуляют по аллеям. Цветы распускаются в садах. Воздух наполнен теплом. Настроение у всех хорошее. Весна радует всех вокруг. День становится длиннее. Ночь становится короче. Жизнь наполняется светом. Люди улыбаются чаще. Природа просыпается ото сна. Птицы вьют гнёзда. Дети запускают кораблики. Вода в реке течёт быстрее. Ветер приносит тепло. Весна дарит радость всем.
```
И вот как выглядет первая страница:
![](https://github.com/roge111/TF-IDF-words/blob/main/tf_idf/Examples/Страница%201.jpg?raw=true)

И вот как выглядет таблица после обработки файла

![](https://github.com/roge111/TF-IDF-words/blob/main/tf_idf/Examples/Страница%202.jpg?raw=true)
...
![](https://media1.tenor.com/m/_U6dCguctMcAAAAC/naza-bart.gif)
